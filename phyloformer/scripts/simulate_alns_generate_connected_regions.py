...
def process_a_tree(row_enum, aln_dir: str, tree_dir: str, dis_mat_dir: str, partial_lh_dir: str, start_seed, num_aln):
    # set random seed num
    ...
    if (not (os.path.isfile(tree_file_full_path) and os.path.getsize(tree_file_full_path) > 0)):
        ...
        # prepare fixed_blength option for IQ-TREE
        global IQ_TREE_PATH
        fixed_blength = "  -blfix "
        ...
        # simulate (20) alignments
        simulated_dir = os.path.join(aln_dir, "../simulated_aln/")
        full_cmd = IQ_TREE_PATH + " --alisim " + os.path.join(simulated_dir, data[1]) + " --num-alignments " + str(num_aln) + " -s " + os.path.join(aln_dir, data[1] + ".phy") + " -te " + tree_file_full_path \
                   + fixed_blength + " -redo -m " + data[3] + " -seed " + str(seed_num) + " -pre " + os.path.join(aln_dir, data[1] + "_" + str(file_index) + ".phy")
        #print(full_cmd)
        ...
        output, error1 = process.communicate()
        ...
        if process.returncode != 0:
            ...
            # replace the input tree by the complete tree
            try:
                complete_tree = os.path.join(aln_dir, data[1] + "_" + str(file_index) + ".phy.treefile")
                shutil.copyfile(complete_tree, tree_file_full_path)
            except:
                # do nothing
                a = 1

            for i in range(1, num_aln + 1):
                full_cmd = IQ_TREE_PATH + " -s " + os.path.join(simulated_dir, data[1] + "_" + str(i) + ".phy") + " -te " + tree_file_full_path \
                           + fixed_blength + " -redo -m " + data[3] + " -keep-ident -num-con-regs " + str(num_con_regs) \
                           + " -con-regions-pref " + os.path.join(tree_dir, "tree_" + data[0] + "_" + str(i)) \
                           + " -dis-mat " + os.path.join(dis_mat_dir, "tree_" + data[0] + "_" + str(i)) + " -partial-lh " + os.path.join(partial_lh_dir, "tree_" + data[0] + "_" + str(i)) \
                           + " -nt 1 --kernel-nonrev -seed " + str(i*1000 + seed_num)
                #print(full_cmd)
                ...
                partial_lh_file_pref = os.path.join(partial_lh_dir, "tree_" + data[0] + "_" + str(i))
        ...
        # remove simulated_aln
        try:
            for file in glob.glob(os.path.join(simulated_dir, data[1] + "_*.phy*")):
                os.remove(file)
        except:
            # do nothing
            a = 1
        ...
        try:
            for file in glob.glob(os.path.join(aln_dir, data[1] + "_" + str(file_index) + ".phy.*")):
                os.remove(file)
        except:
            # do nothing
            a = 1
def process_all_trees(start: int, end: int, in_file: str, aln_dir: str, tree_dir: str, dis_mat_dir: str, partial_lh_dir: str, start_seed, num_aln, nprocesses):
    ...
    with tqdm(total=len(rows)) as pbar:
        for _iter in pool.imap_unordered(partial(process_a_tree, aln_dir = aln_dir, tree_dir = tree_dir, dis_mat_dir = dis_mat_dir, partial_lh_dir = partial_lh_dir, start_seed = start_seed, num_aln = num_aln), enumerate(rows)):
            pbar.update()
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of simulated alignments",
    )
    parser.add_argument(
        "-start",
        "--start",
    ...
    # process all trees
    process_all_trees(args.start, args.end, args.input, args.aln_dir, args.tree_dir, args.dis_mat_dir, args.partial_lh_dir, start_seed, args.num_aln, args.nprocesses)
