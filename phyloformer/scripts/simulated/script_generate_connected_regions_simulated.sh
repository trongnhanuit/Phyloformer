...
for i in {1..7}; do 
	...
	python3 ${PHYLO_FORMER_DIR}${SCRIPTS_DIR}simulate_alns_generate_connected_regions.py -i ${PHYLO_FORMER_DIR}${DATA_DIR}tree_model_wrt_aln_id_filtered.csv -aln ${PHYLO_FORMER_DIR}${DATA_DIR}aln/ -tree ${PHYLO_FORMER_DIR}${DATA_DIR}tree/ -dis_mat_dir ${PHYLO_FORMER_DIR}${DATA_DIR}dis_mat/ -partial_lh_dir ${PHYLO_FORMER_DIR}${DATA_DIR}partial_lhs/full_length/ -seed 1 -num_aln 1 -p $num_cpus -start $start_id -end $end_id &> /scratch/dx61/tl8625/Phyloformer/phyloformer/scripts/script_generate_connected_regions_${range}.log
