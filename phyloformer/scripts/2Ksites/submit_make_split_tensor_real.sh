...
part=1 
	...
	# make tensors
	python3 ${PHYLOFORMER_DIR}scripts/make_tensors.py -t ${PHYLOFORMER_DIR}${DATA_DIR}dis_mat/ -a ${PHYLOFORMER_DIR}${DATA_DIR}partial_lhs/trimmed/ -o ${PHYLOFORMER_DIR}${DATA_DIR}dataset/  -con_reg -p ${num_cpus} &> ${PHYLOFORMER_DIR}${SCRIPTS_DIR}make_split_tensor${DATA_TYPE}_${part}.log
