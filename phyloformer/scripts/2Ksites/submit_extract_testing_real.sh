...
DATA_TYPE="_real"
THRESHOLD=13500 # MAX * 0.38 where MAX = 32767
num_cpus=48
dataset="testing"
...
dataset="testing"
python3 ${PHYLOFORMER_DIR}scripts/search_large_values.py -i ${PHYLOFORMER_DIR}${DATA_DIR}dataset/normalized_${dataset}/ -p $num_cpus -max 1
...
part=1
	...
	python3 ${PHYLOFORMER_DIR}scripts/get_true_trees_testing.py -tree_dir ${PHYLOFORMER_DIR}${DATA_DIR}tree -testing ${PHYLOFORMER_DIR}${DATA_DIR}dataset/normalized_${dataset}/ -true_tree_testing ${PHYLOFORMER_DIR}${DATA_DIR}true_tree_testing -p $num_cpus &> ${PHYLOFORMER_DIR}${SCRIPTS_DIR}get_true_trees_testing${DATA_TYPE}.log
