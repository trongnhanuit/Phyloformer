...
def process_a_tree(row_enum, aln_dir: str, tree_dir: str, dis_mat_dir: str, partial_lh_dir: str, iqtree_path: str, sel_factor: float, start_seed):
    ...
    tree_file_full_path = os.path.join(tree_dir, data[0] + ".nwk")
    if (not (os.path.isfile(tree_file_full_path) and os.path.getsize(tree_file_full_path) > 0)):
        ...
        #global IQ_TREE_PATH
        full_cmd = iqtree_path + " -s " + os.path.join(aln_dir, data[0] + ".fa") + " -te " + tree_file_full_path \
        ...
        try:
            for file in glob.glob(os.path.join(aln_dir, data[0] + ".fa.*")):
                os.remove(file)
        except:
            # do nothing
            a = 1
...
def process_all_trees(start: int, end: int, in_file: str, aln_dir: str, tree_dir: str, dis_mat_dir: str, partial_lh_dir: str, iqtree_path: str, sel_factor: float, start_seed, nprocesses):
    ...
    for index, row in df.iterrows():
        if (start == -1 or index >= start) and (end == -1 or index < end):
            rows.append(str(row['TREE_KEY']) + "\t" + row['ALI_ID'] + "\t" + str(row['TAXA']) + "\t" + row['MODEL'] + "\t" + row['NEWICK_STRING'])
